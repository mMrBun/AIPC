...
from server.utils.collect_tools import get_tools
...
async def _create_local_tool_call_completion(
        request: "ToolCallRequest", model: "ChatModel"
) -> "ChatCompletionResponse":
    msg_id = uuid.uuid4().hex
    input_kwargs = {
        "messages": [dictify(message) for message in request.messages],
...
def launch_server(config: "ChatConfig") -> None:
    ...
    @app.post("v1/chat/tool_call", response_model=ChatCompletionResponse, status_code=status.HTTP_200_OK)
    async def create_tool_call_completion(request: "ToolCallRequest"):
        # todo implement tool call
        return await _create_local_tool_call_completion(request, model)
